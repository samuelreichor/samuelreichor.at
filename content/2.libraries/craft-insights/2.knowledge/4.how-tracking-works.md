...
Available when [LLMify](/libraries/craft-llmify) is installed. Every Markdown response LLMify serves is counted: requests to `.md` pages, `llms.txt`, `llms-full.txt`, content-negotiated swaps, and bot-detected responses.
...
| Type | Definition |
|------|------------|
| **Direct** | Agent requested a Markdown URL explicitly (`/raw/<slug>.md`, `/llms.txt`, `/llms-full.txt`, `/.well-known/llms.txt`) |
| **Negotiated** | Agent requested the regular page URL and LLMify swapped the response to Markdown via content negotiation or bot detection |
